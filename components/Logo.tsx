...
export const Logo: React.FC<LogoProps> = ({
  ...
  // Cor #470082 para fundo claro (variant='purple'), Branco para fundo escuro (variant='white')
  const fillColor = variant === 'white' ? '#ffffff' : '#470082';

  // Default sizing if class not provided
  let defaultSizeClass = "h-10 w-auto";
  if (orientation === 'vertical') defaultSizeClass = "h-12 w-auto";
  if (orientation === 'icon') defaultSizeClass = "h-12 w-12";

  const sizeClass = className || defaultSizeClass;

  // Icon variant (and now used for Horizontal/Default to remove text)
  const IconSvg = (
  ...
  if (orientation === 'vertical') {
    // For now, returning IconSvg for vertical as well to ensure no text is shown
    // If a vertical layout of the icon is needed, it can be adjusted here.
    return IconSvg;
  }
